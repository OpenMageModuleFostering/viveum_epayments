...
class Netresearch_OPS_Model_Observer
{
    ...
    /**
     * Replace order cancel comfirm message of Magento by a custom message from Viveum
     *
    ...
    public function updateOrderCancelButton(Varien_Event_Observer $observer)
    {
        ...
        //If payment method is one of the Viveum-ones and order can be cancelled manually
    ...
    /**
     *
     * appends a checkbox for closing the transaction if it's a Viveum payment
     *
    ...
    public function appendCheckBoxToRefundForm($observer)
    {
         ...
         * the refund can be done online and the payment is done via Viveum
         */
     ...
     * - payment is a Viveum payment
     * - Viveum transaction is closed
     *
     * @event core_block_abstract_to_html_after
     *
     * @param Varien_Event_Observer $observer
    ...
    /**
     * appends the status update button to the order's button in case it's an Viveum payment
     *
    ...
    protected function invokeRequestParamValidation($requestParams)
    {
        ...
        if (false == $validator->isValid($requestParams)) {
            $this->getOnepage()->getCheckout()->setGotoSection('payment');
            Mage::throwException(
                $this->getHelper()->__('The data you have provided can not be processed by Viveum')
            );
        }
    ...
    /**
     * resets the order status back to pending payment in case of directlink payments in Viveum authorize status
     *
    ...
    public function setOrderStateDirectLink(Varien_Event_Observer $event)
    {
        ...
        if ($payment->getMethodInstance() instanceof Netresearch_OPS_Model_Payment_DirectLink
            ...
            $payment->getOrder()->setState(
                ...
                $this->getHelper()->__('Payment has been authorized by Viveum, but not yet captured.')
            );
        }
    }
    ...
    /**
     * appends the resend payment info button to the order's button in case it's an Viveum payment
     ...
     * that no call to Viveum will happen
     *
     * @event        adminhtml_block_html_before
     *
     * @param Varien_Event_Observer $observer
