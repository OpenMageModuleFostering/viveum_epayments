...
class Netresearch_OPS_Model_Payment_Abstract extends Mage_Payment_Model_Method_Abstract
{
    ...
    public function getFormFields($order, $requestParams, $isRequest = true)
    {
        ...
        if ($isRequest) {
            ...
            $helper->log(
                $helper->__(
                    "Register Order %s in Viveum \n\nAll form fields: %s\nViveum String to hash: %s\nHash: %s",
    ...
    public function initialize($paymentAction, $stateObject)
    {
        ...
        $message = $this->getHelper()
            ->__('Customer got redirected to Viveum for %s. Awaiting feedback.', $paymentAction);
    ...
    public function acceptPayment(Mage_Payment_Model_Info $payment)
    {
        ...
        Mage::throwException(
            $this->getHelper()->__(
                'The order can not be accepted via Magento. For the actual status of the payment check the Viveum backend.'
            )
        );
    }
    ...
    {
        // disallow Viveum online capture if amount is zero
        if ($amount < 0.01) {
    ...
    /**
     * Returns the mandatory fields for requests to Viveum
     *
     * @param Mage_Sales_Model_Order $order
     *
     * @return array
    ...
    public function cancel(Varien_Object $payment)
    {
        ...
        if (true === $this->canCancelManually($payment->getOrder())) {
            $payment->getOrder()->addStatusHistoryComment(
                $this->getHelper()->__("The order was cancelled manually. The Viveum-state is 0 or null.")
            );
     ...
     * @param array                  $formFields    Parameters to be sent to Viveum
     *
     * @return bool
     */
    ...
    /**
     * If cart Item information has to be transmitted to Viveum
     *
     * @return bool
     */
    ...
    /**
     * Returns array with the order item data formatted in Viveum fashion if payment method implementation
